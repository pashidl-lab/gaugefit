...
런타임 의존성은 numpy 하나다 — 엔진, 파인더, 레시피 런타임까지 전부. 셋업 때만 쓰는
기능 둘이 더 요구한다. 도트 그리드 타겟 캘리브레이션과 Gage R&R 의 교호작용 F 검정은
SciPy 를 쓰고(`pip install "gaugefit[calib]"`), PNG/PGM 외 포맷은 Pillow 를 쓴다
(`gaugefit[image]`). 둘 다 패키지를 임포트하거나 측정하는 데는 필요 없다.
